...
## Persistent Data: PGlite (Lightweight Deployments)

Dory uses PGlite as a lightweight embedded application database by default. It is a good fit for local evaluation, demos, single-node self-hosting, and small-team trials.

PGlite can persist data through the filesystem, but it is not a traditional multi-connection Postgres server. For formal team production environments, use external Postgres instead.

If you continue using PGlite, mount `/app/data` to persistent disk storage and back up that directory regularly:
...
## Recommended Production Setup: Postgres

For formal team production environments, store Dory application data in external Postgres and manage backups, recovery, monitoring, and permissions for that database:
