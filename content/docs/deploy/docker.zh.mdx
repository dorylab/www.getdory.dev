...
Dory 默认使用 PGlite 作为轻量级内置应用数据库，适合本地评估、Demo、单机自托管和小团队试用。

PGlite 使用文件系统持久化时可以保存数据，但它不是传统多连接 Postgres Server。正式团队生产环境建议使用外部 Postgres。

如果继续使用 PGlite，请务必挂载 `/app/data` 到持久化存储，并定期备份这个目录。
...
如用于正式团队生产环境，推荐使用外部 Postgres 存储 Dory 自己的应用数据，并对数据库进行备份、恢复、监控和权限管理：
